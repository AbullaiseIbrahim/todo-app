...
function Listing() {
    ...
    const handleClick = (index) => {
        const delData = dataSet.map((data, ind) => {
            ...
            if(index === ind && !dltDataValue.isDeleted) dltDataValue.isDeleted = true
            else if(index === ind && dltDataValue.isDeleted) dltDataValue.isDeleted = false
            return {...dltDataValue}
        })
        setDataSet(delData)
    ...
    const handleActiveClick = (index) => {
        const updatedData = dataSet.map((data, ind) => {
            ...
            if (index === ind && !dataSetValue.isSelected) dataSetValue.isSelected = true
            else if (index === ind && dataSetValue.isSelected) dataSetValue.isSelected = false
            return {...dataSetValue}
        })
    ...
    useEffect(() => {
        const toDoListItems = JSON.parse(localStorage.getItem("todo_list_data")) || [];
        ...
        <div className='listing-wrap'>
            <div className='add-list'>
                ...
                <div className='form-input-row'>
                    <input value={toDo} onChange={(e)=>setToDo(e.target.value)} type="text" placeholder='Type here..' />
                    <i onClick={toDo === '' ? '' : handleAddData} className="fa-solid fa-circle-plus"></i>
                </div>
                }
            </div>
            <div className='list-item'>
                <ul className='listTodo'>
                    {
                        dataSet.map(({value, isSelected, isDeleted},index) => {
                                return(
                                    <li className={isDeleted ? 'toDo dltToDo' : 'toDo' && isSelected ? 'toDo actToDo' : 'toDo'} key={index}>
                                        <div className={isSelected ? "input selected" : "input"}>
                                            <input value={value.status} type="checkbox" name="todoCheck" id={index} />
                                            <label id={index} htmlFor={index} onClick={ ()=> handleActiveClick(index) }></label>
                                        </div>
                                        <p>{value}</p>
                                        <div className="dlt-btn">
                                            <i className={isDeleted ? 'fa-solid fa-rotate-left' : 'fa-solid fa-trash-can'} onClick={ ()=> handleClick(index) }></i>
                                        </div>
                                    </li>
                                )
                            }
                ...
                </ul>
                {
                    ...
                    <div className="clear-button">
                        <button onClick={rmLocalStorage} > Clear </button>
                    </div>
                }
            </div>
        </div>
